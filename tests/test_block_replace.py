...
def test_delete_title_strip_keeps_outside_text():
    ...
    msp = doc.modelspace()
    fb = (0, 0, 200, 100)
    add_text(msp, "标题内", 150, 10)   # 在标题区 -> 删
    add_text(msp, "视图区", 30, 80)    # 图内 -> 留
...
def test_delete_frame_border_only_matching_closed_rect():
    ...
    add_rect(msp, 10, 10, 60, 60, closed=True)     # 不重合 -> 留
