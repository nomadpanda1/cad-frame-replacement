...
<ul>
  ...
  <li>案例十输出（成品 DWG，11 张，本地运行生成，体积大不入库）：<a href="../../output_test/">output_test/（*_HH.dwg）</a> / 案例十说明：<a href="10_residential_electrical/summary.md">summary.md</a></li>
...
<p><span class="tag">11 张 DWG</span><span class="tag">天面/强弱电/消防/系统/裙楼/首二层/高低压</span><span class="tag">打散图框</span><span class="tag">中文 SHX</span> 来源：爱给网住宅楼电气设计方案（AutoCAD/ZWCAD，提供 dwg）。全部为真实设计院图纸、打散图框（0 个 INSERT 块式标题栏）、含中文 SHX 字体——与微信发来的图纸同属「打散」一类。逐张走「块式 0 命中 → 线框检测回退」：删旧外框+标题栏+边缘区号 → 整图幅插公司图框 → 回填。本套 11/11 全部成功（HH_FRAME 块已插入、字段回填、旧图框残留 = 0），并新增符号块误检过滤（消防系统图中的 M_I14YDH 符号块被正确过滤）。成品 DWG 在 <code>output_test/*_HH.dwg</code>。</p>
